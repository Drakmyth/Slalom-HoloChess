...
public class SelectionMenu : MonoBehaviour {
    ...
    public static Dictionary<int, Dictionary<int, Dictionary<AttackResult, decimal>>> ResultAverages { get; set; }

    // Use this for initialization
    void Start () {
        InitializeResultAverages();
    ...
    private string GetAttackResultPercentString(int attack, int defense, AttackResult attackResult)
    {
        string percent = "";
        // ensure attack result exists
        if (ResultAverages.ContainsKey(attack) && ResultAverages[attack].ContainsKey(defense) && ResultAverages[attack][defense].ContainsKey(attackResult))
        {
            percent = ResultAverages[attack][defense][attackResult] * 100 + "%";
        }

        return percent;
    }

    private void InitializeResultAverages()
    {
